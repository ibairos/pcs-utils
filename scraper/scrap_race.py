...
def scrap_items(url: str, results_len: int) -> list:
    ...
    driver.get(url)
    # Get results
    result_items = driver.find_element_by_xpath("//div[contains(@class, 'result-cont') and not(contains(@class, 'hide'))]").find_element_by_xpath(".//table[contains(@class, 'basic') and contains(@class, 'results')]").find_element_by_xpath(".//tbody").find_elements_by_xpath(".//tr")
    results = list()
    for index in range(0, results_len):
        result_link = result_items[index].find_element_by_xpath(".//a")
        results.append({
